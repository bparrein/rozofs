...
static char ref_support[] = {
    3,1,4,1,5,9,2,6,
    5,3,5,8,9,7,9,3,
    2,3,8,4,6,2,6,4
};
static char ref_p0_bins[] = {2, 6, 8, 0, 10, 10, 4, 4, 1, 6};
static char ref_p1_bins[] = {4, 1, 9, 13, 10, 12, 13, 1};
static char ref_p2_bins[] = {3, 4, 5, 7, 5, 4, 3, 13, 5, 4};

static char *support = NULL;
...
int test_transform_initialize(void) {
    ...
    projections[0].bins = malloc(10 * sizeof(char));
    ...
    projections[1].bins = malloc(8 * sizeof(char));
    ...
    projections[2].bins = malloc(10 * sizeof(char));
...
int test_transform_forward(void) {
    ...
    memcpy(support, ref_support, 24 * sizeof(char));

    if (transform_forward(support, 3, 8, 3, projections) != 0) {
	status = -1;
	goto out;
    }

    status = -!(memcmp(projections[0].bins, ref_p0_bins, 10 * sizeof(char)) == 0 &&
	memcmp(projections[1].bins, ref_p1_bins, 8 * sizeof(char)) == 0 &&
	memcmp(projections[2].bins, ref_p2_bins, 10 * sizeof(char)) == 0);
out:
...
int test_transform_inverse(void) {
    ...
    memcpy(projections[0].bins, ref_p0_bins, 10 * sizeof(char));
    memcpy(projections[1].bins, ref_p1_bins, 8 * sizeof(char));
    memcpy(projections[2].bins, ref_p2_bins, 10 * sizeof(char));

    if (transform_inverse(support, 3, 8, 3, projections) != 0) {
	status = -1;
	goto out;
    }

    status = -!(memcmp(support, ref_support, 24 * sizeof(char)) == 0);
out:
    if (status != 0) errno = EPERM;
    return status;
}
