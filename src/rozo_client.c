...
int rozo_client_statfs(rozo_client_t *rozo_client, struct statvfs *st) {
    ...
    if ((response = exportproc_statfs_1(rozo_client->export_uuid, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
int rozo_client_stat(rozo_client_t *rozo_client, const char *path, struct stat *st) {
    ...
    if ((response = exportproc_stat_1(&args, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
int rozo_client_readlink(rozo_client_t *rozo_client, const char *target, char *link) {
    ...
    if ((response = exportproc_readlink_1(&args, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
int rozo_client_mknod(rozo_client_t *rozo_client, const char *path, mode_t mode) {
    ...
    if ((response = exportproc_mknod_1(&args, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
int rozo_client_mkdir(rozo_client_t *rozo_client, const char *path, mode_t mode) {
    ...
    if ((response = exportproc_mkdir_1(&args, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
rozo_dirent_t * rozo_client_readdir(rozo_client_t *rozo_client, const char *path) {
    ...
    if ((response = exportproc_list_1(&args, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
int rozo_client_unlink(rozo_client_t *rozo_client, const char *path) {
    ...
    if (S_ISREG(st.st_mode)) {
        if ((export_attr_response = exportproc_attr_1(&args, rozo_client->export_client.client)) == NULL) {
            status = -1;
            goto out;
        }
        ...
        for (i = 0; i < ROZO_SAFE; i++) {
            ...
            uuid_copy(storage_remove_args.uuid, export_attr_response->export_attr_response_t_u.attr.uuids[i]);
            uuid_copy(storage_remove_args.mf, export_attr_response->export_attr_response_t_u.attr.uuid);
            if (rpc_client_initialize(&client, export_attr_response->export_attr_response_t_u.attr.hosts[i],
                    STORAGE_PROGRAM, STORAGE_VERSION, 0, 0) == 0) {
                storageproc_remove_1(&storage_remove_args, client.client);
                rpc_client_release(&client);
            }
        }
    }

    if ((export_status_response = exportproc_unlink_1(&args, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
int rozo_client_rmdir(rozo_client_t *rozo_client, const char *path) {
    ...
    if ((response = exportproc_rmdir_1(&args, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
int rozo_client_symlink(rozo_client_t *rozo_client, const char *target, const char *link) {
    ...
    if ((response = exportproc_symlink_1(&args, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
int rozo_client_rename(rozo_client_t *rozo_client, const char *from, const char *to) {
    ...
    if ((response = exportproc_rename_1(&args, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
int rozo_client_chmod(rozo_client_t *rozo_client, const char *path, mode_t mode) {
    ...
    if ((response = exportproc_chmod_1(&args, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
int rozo_client_truncate(rozo_client_t *rozo_client, const char *path, uint64_t offset) {
    ...
    if ((response = exportproc_trunc_1(&args, rozo_client->export_client.client)) == NULL) {
        status = -1;
        goto out;
    }
...
static int disconnect_file(rozo_file_t *file) {
    ...
    for (i = 0; i < ROZO_SAFE; i++) {
        rpc_client_release(&file->storages[i]);
    }
...
static int connect_file(rozo_file_t *file) {
    ...
    for (i = 0; i < ROZO_SAFE; i++) {
        ...
        if (rpc_client_initialize(&file->storages[i], file->hosts[i],
            ...
            warning("rpc_client_initialize failed: storage server: %s unreachable, %s",
...
rozo_file_t * rozo_client_open(rozo_client_t *rozo_client, const char *path, mode_t mode) {
    ...
    if ((response = exportproc_attr_1(&args, rozo_client->export_client.client)) == NULL) {
        rozo_client_close(file);
        file = NULL;
        goto out;
    }
...
static int read_blocks(rozo_client_t *rozo_client, rozo_file_t *file, int64_t mb, uint32_t nmbs, char *data) {
    ...
    export_read_block_response = exportproc_read_block_1(&export_read_block_args, rozo_client->export_client.client);
    ...
    if (export_read_block_response == NULL || export_read_block_response->status == EXPORT_FAILURE) {
        if (export_read_block_response != NULL) {
            severe("read_blocks failed: export read block response failure (%s)"
                    , strerror(export_read_block_response->export_read_block_response_t_u.error));
        } else {
            // If no response from export server during TIMEOUT seconds
            warning("read_blocks failed: export read block (no response from export server)");
        }
        errno = EIO; // I/O error
        status = -1;
        goto out;
    ...
    distribution = export_read_block_response->export_read_block_response_t_u.distribution.distribution_val;
    ...
    while (i < nmbs) {
        ...
        for (mp = 0; mp < ROZO_FORWARD; mp++) {
            ...
            DEBUG("read_blocks: (block %llu to block %llu) projection: %d is on index: %d",
            ...
            if (storage_read_response == NULL || storage_read_response->status == STORAGE_FAILURE) {
                if (storage_read_response != NULL) {
                    severe("read_blocks failed: storage read response failure (%s)"
                            , strerror(storage_read_response->storage_read_response_t_u.error));

                    xdr_free((xdrproc_t) xdr_storage_read_response_t, (char *) & storage_read_response);
                } else {
            ...
            if ((bins[connected] = malloc(storage_read_response->storage_read_response_t_u.bins.bins_len)) == NULL) {
                status = -1;
                goto out;
            }

            // Copy received bins
            memcpy(bins[connected], storage_read_response->storage_read_response_t_u.bins.bins_val,
                    storage_read_response->storage_read_response_t_u.bins.bins_len);
            ...
            xdr_free((xdrproc_t) xdr_storage_read_response_t, (char *) & storage_read_response);
            free(storage_read_response->storage_read_response_t_u.bins.bins_val);

            // Increment the number of received requests (and stop the loop if enough request received)
            if (++connected == ROZO_INVERSE) break;
        }

        // Not enough server storage response to retrieve the file
        if (connected < ROZO_INVERSE) {
            severe("read_blocks failed: error not enough response from storage servers for retrieve the blocks"
                    " %llu to %llu of file: %s (only %d/%d)", mb, mb + nmbs, file->path, connected, ROZO_INVERSE);
            errno = EIO; // I/O error
            status = -1;
        ...
        for (j = 0; j < n; j++) {
            ...
            if (transform_inverse(data + (ROZO_BSIZE * (i + j)), ROZO_INVERSE, ROZO_BSIZE / ROZO_INVERSE, ROZO_INVERSE,
                    projections) != 0) {
                severe("read_blocks failed: inverse data failure for block %llu of file: %s", (mb + i + j), file->path);
                errno = EIO;
                status = -1;
                goto out;
            }
...
out:
    // Free the memory area where are stored the bins used by the inverse transform
    for (mp = 0; mp < ROZO_INVERSE; mp++) if (bins[mp] != NULL) free(bins[mp]);
    // Free the memory area where the request send to storage server is stored
    xdr_free((xdrproc_t) xdr_storage_read_args_t, (char *) & storage_read_args);
...
int64_t rozo_client_read(rozo_client_t *rozo_client, rozo_file_t *file, uint64_t off, char *buf, uint32_t len) {
    ...
    export_io_response = exportproc_read_1(&export_io_args, rozo_client->export_client.client);

    if (export_io_response == NULL || export_io_response->status == EXPORT_FAILURE) {
        length = -1;
        goto out;
    }
    ...
    last = (off + length) / ROZO_BSIZE + ((off + length) % ROZO_BSIZE == 0 ? -1 : 0);
    loffset = (off + length) - last * ROZO_BSIZE;

    DEBUG("rozo_client_read: %lld, first: (%lld, %d), last: (%lld, %d)", length, first, foffset, last, loffset);
...
out:
    ...
    if (export_io_response != NULL)
        xdr_free((xdrproc_t) xdr_export_io_response_t, (char *) export_io_response);
    return length;
}

static int write_blocks(rozo_client_t *rozo_client, rozo_file_t *file, uint64_t mb, uint32_t nmbs, const char *data) {
    ...
    for (i = 0; i < nmbs; i++) {
        ...
        for (mp = 0; mp < ROZO_FORWARD; mp++) {
            ...
            projections[mp].bins = storage_write_args[mp].bins.bins_val + (rozo_psizes[mp] * i * sizeof (uint8_t));
        }

        // Apply the erasure code transform for the block i
        if (transform_forward(data + (i * ROZO_BSIZE), ROZO_INVERSE, ROZO_BSIZE / ROZO_INVERSE,
                ROZO_FORWARD, projections) != 0) {
            severe("write_blocks failed: transform data failure for block %llu of file: %s", (i + mb), file->path);
            errno = EIO; // I/O error
            status = -1;
            goto out; // -> test to reconnect the file (in function rozo_client_write())
        }
    ...
    for (ps = 0; ps < ROZO_SAFE; ps++) {
        ...
        if (response == NULL || response->status == STORAGE_FAILURE) {
            if (response != NULL) {
                severe("write_blocks failed: storage write response failure (%s)"
    ...
    if (export_response == NULL || export_response->status == EXPORT_FAILURE) {
        if (export_response != NULL) {
            severe("write_blocks failed: export write block response failure (%s)"
                    , strerror(export_response->export_status_response_t_u.error));
        } else {
            // If no response from export server during TIMEOUT seconds
            warning("write_blocks failed: export write block (no response from export server)");
        }
        errno = EIO; // I/O error
        status = -1;
        goto out;
...
out:
    ...
    for (mp = 0; mp < ROZO_FORWARD; mp++) {
        if (&storage_write_args[mp] != NULL) {
            xdr_free((xdrproc_t) xdr_storage_write_args_t, (char *) &storage_write_args[mp]);
        }
    }
    // Free the memory area where the request send to export server is stored
    xdr_free((xdrproc_t) xdr_export_write_block_args_t, (char *) &export_write_block_args);
    // Free the memory area where the response from the export server is stored
    if (export_response != NULL)
        xdr_free((xdrproc_t) xdr_export_status_response_t, (char *) export_response);
    return status;
}
...
int64_t rozo_client_write(rozo_client_t *rozo_client, rozo_file_t *file, uint64_t off, const char *buf, uint32_t len) {
    ...
    DEBUG("rozo_client_write: %lld, first: (%lld, %d), last: (%lld, %d)", length, first, foffset, last, loffset);
    ...
    export_io_response = exportproc_write_1(&export_io_args, rozo_client->export_client.client);
